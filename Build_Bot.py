...
+  make setup script that gets you all blocks 
_  add more colors as options
?  try correcting pytesseract errors when reading weird text
+  MAKE IT FASTER. make it work backwards on odd rows
+  add a fast option where it inputs setblock commands
'''
...
def getWrittenCommand(block,i,j):
    mycommand = "setblock ~" + str(i + 2) + " ~ ~" + str(j + 2) + " minecraft:"
    if "Wool" in block:
        mycommand += "wool "
